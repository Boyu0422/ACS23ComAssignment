...
i64 errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern i64 syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern i64 line; /* source code line number - defined in scanner.c */
...
extern bust_void startParser(bust_void);
extern i64 startScanner(ReaderPointer sc_buf);

static bust_void printParserError(String fmt, ...);
static bust_void displayParser(ReaderPointer ptrBuffer);
static bust_long getParserFileSize(String fname);
static bust_void callGarbageCollector(bust_void);
...
bust_void printParserError(String fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(bust_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
bust_long getParserFileSize(String fname) {
	FILE* input;
	bust_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
bust_void displayParser(ReaderPointer ptrBuffer) {
...
bust_void callGarbageCollector(bust_void) {
